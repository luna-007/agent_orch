...
from schemas.tool_schemas import DiskQueryInput, Message
...
async def test_workflow_builder_validation_and_building():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        builder = WorkflowBuilder(temp_dir, registry, orchestrator)
        ...
        # 2. Write invalid manifest referencing unknown tool
        invalid_tool_manifest = VALID_MANIFEST.copy()
        invalid_tool_manifest["metadata"] = {"name": "invalid_tool_flow", "version": "1.0.0"}
        invalid_tool_manifest["agents"] = {
            "test_agent": {
                "system_prompt": "Prompt",
                "tools": ["unknown_tool"] # Does not exist in registry
            }
        }
        
        with open(os.path.join(temp_dir, "invalid_tool_flow.json"), "w") as f:
            json.dump(invalid_tool_manifest, f)
            
        with pytest.raises(ValueError) as exc:
            WorkflowBuilder(temp_dir, registry, orchestrator)
        assert "references tool 'unknown_tool' which does not exist" in str(exc.value)
